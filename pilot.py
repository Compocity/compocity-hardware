...
def dht():
    result = 'hello'
    humidity, temperature = Adafruit_DHT.read_retry(DHT_SENSOR, DHT_PIN, 3)
...
def error_text(error):
    ...
    draw.text((0, 30), '{} error!'.format(error), fill = "CYAN", font = font)
    OLED.Display_Image(image)
    OLED.Delay(4000)
    OLED.Clear_Screen()
...
def close_text():
    ...
    draw.text((0, 30), 'Helyezd be a', fill = "CYAN", font = font)
    draw.text((50, 42), 'koposztot!', fill = "CYAN", font = font)
    draw.text((0, 60), 'Aztán csukd le a', fill = "CYAN", font = font)
    draw.text((50, 72), 'Compocity-t!', fill = "CYAN", font = font)
    OLED.Display_Image(image)
